...
/* Step 2: Make sure that the paddle for Player 1 stays on the canvas. Use the constrain () function to keep the center of the left paddle between the top and bottom of the canvas. It is okay and expected that part of the left paddle will be able to go off the canvas when the paddle approaches the top and bottom edges of the canvas. This is okay as long as the center of the paddle still remains on the canvas.*/

//Constrain the player movement
    player1Y = constrain(player1Y,0,400); // Step 2
...
/* Step 1:  Add logic to the draw () function so that: - When the UP arrow key is pressed, te paddle on the left moves up. - When the DOWN arrow key is pressed, the paddle on the left moves down. - When no key is pressed, the paddle on the left does not move.*/

draw = function() {

//Control Player 1 
      if(keyIsPressed){  // Step 1
         if (keyCode === UP) {   // Step 1
            movePlayerUp();   // Step 1
        }
        else if (keyCode === DOWN) {  // Step 1
            movePlayerDown();  // Step 1
        }
    }
